...
while cap.isOpened():
    ...
    if ret:
        ...
        cv2.putText(frame, translated_text, (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
...
def recognize_and_translate():
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            speech_text = recognizer.recognize_google(audio, language=source_lang)
            print(f"Recognized Speech: {speech_text}")

            # Translate the recognized speech
            translated_text = translator.translate(speech_text, src=source_lang, dest=target_lang).text
            print(f"Translated Text: {translated_text}")
...
while(cap.isOpened()):
    ret, frame = cap.read()
    if ret:
        # Recognize and translate speech every certain number of frames
        speech_text, translated_text = recognize_and_translate()

        # Overlay the text on the video frame
        if speech_text and translated_text:
            cv2.putText(frame, speech_text, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, translated_text, (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
